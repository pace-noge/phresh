...
class TestEvaluationRoutes:
    async def test_routes_exists(self, app: FastAPI, client: AsyncClient) -> None:
        ...
        assert res.status_code != status.HTTP_404_NOT_FOUND
        res = await client.get(app.url_path_for("evaluations:get-stats-for-cleanerr", username="bradpitt"))
        assert res.status_code != status.HTTP_404_NOT_FOUND
...
class TestCreateEvaluations:
    ...
    async def test_owner_cant_leave_review(
            self,
            app: FastAPI,
            create_authorized_client: Callable,
            test_user4: UserInDB,
...
class TestGetEvaluations:
    ...
    Test that authenticated user can fetch all of cleaner's evaluations
    Test that a cleaner's evaluations comes with an aggregates.
    """
    async def test_authenticated_user_can_get_evaluation_for_cleaning(
        ...
        res = await authorized_client.get(
            app.url_path_for(
                ...
                cleaning_id=str(test_list_of_cleanings_with_evaluated_offer[0].id),
                username=test_user3.username
            )
        )
        assert res.status_code == status.HTTP_200_OK
    ...
    async def test_authenticated_user_can_get_list_of_evaluation_for_cleaner(
            self,
            app: FastAPI,
            create_authorized_client: Callable,
            test_user3: UserInDB,
    ...
    async def test_authenticated_user_can_get_aggregate_stats_for_cleaner(
        ...
        assert (
                mean([e.professionalism for e in evaluations if
                      e.professionalism is not None]) == stats.avg_professionalism
        )
