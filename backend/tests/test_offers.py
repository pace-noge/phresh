...
class TestGetOffers:
    async def test_cleaning_owner_can_get_offer_from_user(
        ...
        res = await authorized_client.get(
            app.url_path_for(
                ...
                cleaning_id=str(test_cleaning_with_offers.id),
    ...
    async def test_other_authenticated_user_cant_view_offer_from_user(
            self,
            app: FastAPI,
            create_authorized_client: Callable,
            test_user_list: List[UserInDB],
...
class TestAcceptOffers:
    ...
    async def test_owner_cant_accept_multiple_offers(
            self,
            app: FastAPI,
            create_authorized_client: Callable,
            test_user2: UserInDB,
...
class TestCancelOffers:
    async def test_user_can_cancel_offer_after_it_has_been_accepted(
        ...
        res = await accepted_user_client.put(
            app.url_path_for("offers:cancel-offer-from-user", cleaning_id=str(test_cleaning_with_accepted_offer.id))
        )
        assert res.status_code == status.HTTP_200_OK
        canceled_offer = OfferPublic(**res.json())
        assert canceled_offer.status == "cancelled"
        assert canceled_offer.user_id == test_user3.id
        assert canceled_offer.cleaning_id == test_cleaning_with_accepted_offer.id
    ...
    async def test_only_accepted_offers_can_be_cancelled(
        ...
        res = await selected_user_client.put(
            app.url_path_for(
                ...
                cleaning_id=str(test_cleaning_with_accepted_offer.id)
            )
        )
        assert res.status_code == status.HTTP_400_BAD_REQUEST
...
class TestRescindingOffer:
    ...
    async def test_users_cannot_rescind_accepted_offers(
        ...
        authorized_client = create_authorized_client(user=test_user3)
        res = await authorized_client.delete(
            app.url_path_for("offers:rescind-offer-from-user", cleaning_id=str(test_cleaning_with_accepted_offer.id))
        )
        assert res.status_code == status.HTTP_400_BAD_REQUEST
